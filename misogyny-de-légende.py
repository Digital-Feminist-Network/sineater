...
def classify_misogyny(comments, model_pipeline):
    """Classifies misogyny and non-misogyny scores for each comment."""
    misogyny_scores = []
    labels = []
...
def main(sheet_id, gid, column, json_key_file, batch_size):
    ...
    if "Label" not in headers:
        worksheet.update_cell(1, len(headers) + 1, "Misogyny Label")
        headers.append("Misogyny Label")
    if "Score" not in headers:
        worksheet.update_cell(1, len(headers) + 1, "Misogyny Score")
        headers.append("Misogyny Score")

    label_col_index = headers.index("Misogyny Label") + 1
    score_col_index = headers.index("Misogyny Score") + 1
    ...
    with alive_bar(len(records), title="Classifying and Updating") as bar:
        for i, row in enumerate(records):
            ...
            if row.get("Misogyny Label") and row.get("Misogyny Score") is not None:
                bar()
                continue
